<script>
  ...
  let form = {
    ...
    tbakTgl: "", tbakJam: "", tbakPenerima: "", pemberiInstruksi: "" // 🔥 DITAMBAHKAN: state pemberiInstruksi
  };
  ...
  // 🔥 FUNGSI BARU: CETAK LANGSUNG DARI RIWAYAT 🔥
  function cetakDariRiwayat(data) {
    editDataSBAR(data); // Muat data ke form secara otomatis
    setTimeout(() => {
      cetakSBAR(); // Eksekusi cetak setelah data masuk ke form
    }, 300); // Jeda sejenak agar UI sempat render
  }
  ...
  async function kirimDataSBAR() {
    ...
    // 🔥 DITAMBAHKAN: pemberiInstruksi ke dalam gabungan string TTD
    ...
    try {
      if (editRow) {
        ...
        if (error) throw error;
        alert("Sukses! Data SBAR berhasil disimpan ke Supabase.");
  ...
  function editDataSBAR(data) {
    ...
    form.pemberiInstruksi = parts[4] || form.ttdPenerima || ""; // 🔥 DITAMBAHKAN: Ambil data pemberi instruksi
  ...
  <div class="kertas-ui relative bg-white p-6 md:p-10 shadow-md rounded-md max-w-4xl mx-auto">
    {#if isSaving}
      <div class="absolute inset-0 bg-white/80 z-50 flex flex-col justify-center items-center rounded-md">
        ...
        <h3 class="text-blue-600 font-bold mt-4">Menyimpan ke SATSET...</h3>
      </div>
    {/if}

    <div class="text-right text-[8pt] mb-1 font-bold text-gray-500">Form/087/UKP/II/2022/rev 1</div>
      ...
      <tbody>
        <tr>
          <td width="25%" rowspan="2" class="text-center align-middle p-2.5 border border-black">
            ...
            <b class="text-[9pt]">UPT PUSKESMAS<br>PONCOKUSUMO</b>
          </td>
            ...
            <table class="sbar-tabel-tanpa-garis w-full p-1.5 h-full">
              <tbody>
                ...
                <tr>
                  ...
                  <td class="align-top">: <input type="text" bind:value={form.penerima} on:input={() => { form.ttdPenerima = form.penerima; form.pemberiInstruksi = form.penerima; }} class="sbar-input w-[80%]" placeholder="..."></td>
                </tr>
              </tbody>
            </table>
          </td>
